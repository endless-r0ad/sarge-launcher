...
use tauri::{Emitter, AppHandle, WebviewWindow, Manager};

use crate::config::{Q3Browser, AppData};
use crate::server::Quake3Server;

const GETSTATUS: &[u8] = b"\xff\xff\xff\xffgetstatus\x00";
// 'xxxxgetservers {optional: game name} 68 full empty'


#[tauri::command(async)]
pub async fn refresh_all_servers(app: AppHandle, mut all_servers: Vec<Quake3Server>, num_threads: usize, timeout: u64) -> Result<Vec<Quake3Server>, String> {
    ...
    for t in 0..num_threads {
        ...
        handles.push(
            thread::spawn(move || {         
                let socket = UdpSocket::bind("0.0.0.0:0").unwrap(); // make the socket per thread
                ...
                while serv_counter < servers_slice.len() {
                    ...
                    match response {
                        Ok((_bytes, _src)) => {
                            queried_server.ping = ping_start.elapsed().as_millis();
                            queried_server.parse_server_response(&response_buf).unwrap_or_else(|e|{ queried_server.errormessage = e });
                            refreshed.lock().unwrap().push(queried_server);
                        }
                        Err(err) => {
                            queried_server.set_error(err);
...
pub async fn refresh_single_server(mut refresh_server: Quake3Server, timeout: u64, window: WebviewWindow) -> Result<(), String> {
    ...
    match response {
        Ok((_bytes, _src)) => {
            //println!("Thread {}. IP {}. PORT {}. Length of bytes received {}", t, queried_server.ip, queried_server.port, bytes);
            refresh_server.ping = ping_start.elapsed().as_millis();

            refresh_server.parse_server_response(&response_buf).unwrap_or_else(|e|{ refresh_server.errormessage = e });
            let _ = window.emit("q3_single_server", refresh_server).unwrap();
        }
        Err(err) => {
            refresh_server.set_error(err);
...
fn get_user_servers(servers: &mut Vec<Quake3Server>, app_data: &AppData) -> () {
    ...
    for serv in &app_data.custom {
        if !all_servers_addresses.contains(serv) {
            ...
            let mut custom_server = Quake3Server::new(
                ip_port[0].to_string(),
                ip_port[1].to_string(),
                None,
                None
    ...
    for serv in &app_data.pinned {
        if !all_servers_addresses.contains(serv) {
            let ip_port: Vec<&str> = serv.as_str().split(":").collect();
            let mut pinned_server = Quake3Server::new(
                ip_port[0].to_string(),
                ip_port[1].to_string(),
                None,
                None
